...
Chrome拡張機能の最初の動作スライスです。現段階では、カテゴリ名の正規化・重複判定・CRUD、保存形式の検証、予定入力フォームと入力検証、Calendar APIのカレンダー・色一覧取得・予定登録基盤、Manifest V3のポップアップ起動を提供します。OAuthの実Chrome接続と画面からの登録結合は次の段階です。
